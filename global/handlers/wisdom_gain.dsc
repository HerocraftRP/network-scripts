...
use_knowledge_points:
  ...
  tab completions:
    ...
    3: from
    4: <player.target.flag[data.name]||<list[target|npc]>>
  script:
    ...
    - if <player.flag[character.capabilities.<[capability]>].add[<[amount]>]> > 500:
      - narrate "<&c>You are too proficient to learn that much knowledge in <[capability]>."
      - stop
    - if <[target]> == target:
      - define target <player.target||null>
    - else if <[target]> == npc:
      - define target <player.location.find_entities.within[5].filter[has_flag[tutor]].first||null>
    - else:
    ...
    - if <[target].entity_type> != PLAYER:
      ...
      - if <[target].flag[tutor_level]> < <player.flag[character.capabilities.<[capability]>].add[<[amount]>]>:
        - narrate "<&c>They are not skilled enough to teach you more <&e><[capability]>"
        - narrate "<&e>This tutor can only teach up to <[target].flag[tutor_level]> capability"
        - stop
