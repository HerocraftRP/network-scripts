start_timed_action:
 type: task
 debug: false
 definitions: action|duration|callback|callback_data|can_move|can_swap_items|must_stay_sneak|distance_from_origin|must_stare_at_target|must_stare_at_block|must_stare|target|animation_task|cancel_script|target
 script:
    - if <[target].exists>:
