player_join:
  ...
  events:
    ...
    on player quits:
      ...
      - adjust <player> location:<server.flag[join_location.main]>
      - if <[target]> == null:
        - stop
...
set_character_traits:
  ...
  script:
    ...
    - foreach <player.flag[character.clothing].keys>:
      ...
      - adjust <player> cosmetic_armor:<[slot]>|<item[<[clothing]>].with_flag[run_script:cancel]>
    # Wings/Elytra
    ...
    # Health
    - run endurance_gain_<player.flag[character.capabilities.endurance].div[25].round||0>