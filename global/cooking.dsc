...
cooking_refill_purifier:
  type: task
  debug: false
  script:
    - wait 1t
    - stop if:<player.open_inventory.title.equals[Chest].not>
    - inventory set slot:2 o:<item[charcoal].with[quantity=64].with_flag[run_script:cancel]> d:<player.open_inventory>
...
cooking_free_slop_callback:
  ...
  script:
    - if <player.item_in_hand.material.name> != air:
      - narrate "<&c>You need an empty hand for this."
      - stop
    - if !<player.inventory.contains_item[COIN_COPPER].quantity[3]>:
      - narrate "<&e>You don't seem to have enough cash."
      - stop
...
cooking_free_slop_toggle:
  ...
  script:
    ...
    - else:
      ...
      - adjust <context.location.add[1,1,1]> "sign_contents:<&e>Good Slop|Cooks|are away|<&6>3 Copper"
