...
sql_set_inventory:
  ...
  debug: false
  script:
    - ~sql id:players "update:UPDATE player_inventory SET inventory = '<player.inventory.map_slots.escaped>' WHERE uuid = '<player.uuid>_<player.flag[data.name].escaped>';"
...
sql_set_inventory2:
  ...
  debug: false
  script:
    - ~sql id:players "update:UPDATE player_inventory SET uuid = '<player.uuid>', inventory = '<player.inventory.map_slots.escaped>', curios = '<player.items.to_json>' WHERE uuid = '<player.uuid>';"

sql_get_inventory:
  type: task
  debug: false
  script:
    - ~sql id:players "query:SELECT inventory FROM player_inventory WHERE uuid = '<player.uuid>_<player.flag[data.name].escaped>';" save:inv
...
sql_init_player_data:
  ...
  debug: false
  script:
    - ~sql id:players "update:INSERT INTO player_data (uuid, data) VALUES ('<player.uuid>', '<player.flag[data].to_json.escaped>') ON DUPLICATE KEY UPDATE uuid = values(uuid);"

sql_set_player_data:
  type: task
  debug: false
  script:
    - ~sql id:players "update:UPDATE player_data SET data = '<player.flag[data].to_json.escaped>' WHERE uuid = '<player.uuid>';"
...
sql_init_character_data:
  ...
  script:
    - flag <player> sql_init.inventory
    - ~sql id:players "update:INSERT INTO character_data (uuid, data) VALUES ('<player.uuid>_<player.flag[data.name].escaped>', '<player.flag[character].to_json.escaped>') ON DUPLICATE KEY UPDATE uuid = values(uuid);"

sql_set_character_data:
  type: task
  debug: false
  script:
    - ~sql id:players "update:UPDATE character_data SET data = '<player.flag[character].to_json.escaped>' WHERE uuid = '<player.uuid>_<player.flag[data.name].escaped>';"

sql_get_character_data:
  type: task
  debug: false
  script:
    - ~sql id:players "query:SELECT data FROM character_data WHERE uuid = '<player.uuid>_<player.flag[data.name].escaped>';" save:inv
    - if <entry[inv].result_list.first.first.unescaped||null> == null:
      - stop
