...
death_system_player_pickup:
  ...
  script:
    ...
    - if <[corpse].has_flag[carried_by]> && <[corpse].flag[carried_by].entity_type> == PLAYER:
      ...
      - stop
    - else if <[corpse].flag[carried_by].entity_type.starts_with[ASTIKOR]>:
      - mount cancel <[corpse]>
    # Data Flags
    - if !<script.data_key[data.can_carry.<player.flag[character.race]>.<[corpse].flag[race]>]>:
      ...
      - stop
    ## Hacky Shit for CarryOn Scripting Compatability
    - adjust <[corpse]> custom_name:<[corpse].flag[race]>$<player.flag[character.race]>
...
death_system_reset_carrier:
  ...
  script:
    ...
    - run add_framework_flag def:modded_player_right_clicks|cancel|<[target]>
...
death_system_check:
  ...
  script:
    ...
    - else if <context.location.exists> && <context.location.has_flag[respawn_point]>:
      - carry drop location:<context.location.flag[respawn_point]>
    - determine cancelled
