...
block_properties:
  ...
  debug: false
  events:
    on player places block location_flagged:on_place bukkit_priority:HIGHEST ignorecancelled:true:
        - if <context.location.flag[on_place].object_type> == List:
          - foreach <context.location.flag[on_place]>:
            - inject <[value]>
        - else:
          - inject <context.location.flag[on_place]>
    on player breaks block location_flagged:on_break bukkit_priority:HIGHEST ignorecancelled:true:
    ...
    on player steps on block location_flagged:on_step ignorecancelled:true:
    ...
    on player left clicks block location_flagged:on_left_click ignorecancelled:true:
    ...
    on player teleports location_flagged:on_teleport:
        - if <context.location.flag[on_redstone].object_type> == List:
          - foreach <context.location.flag[on_redstone]>:
            - inject <[value]>
        - else:
          - inject <context.location.flag[on_redstone]>
    on block drops item from breaking location_flagged:on_break_drop:
        - if <context.location.flag[on_break_drop].object_type> == List:
          - foreach <context.location.flag[on_break_drop]>:
            - inject <[value]>
        - else:
          - inject <context.location.flag[on_break_drop]>